...
public class TvShowAPI implements ITvShowAPI {
	...
	public String getCrewsListJsonResult(int sid) {
		...
		String urlString = url+"shows/"+sid+"/crew";
		String crews;
		try {
			crews = this.restTemplate.getForObject(urlString, String.class, 200);
	...
	public List<Crew> getCrewsList(int sid) {
		String json = this.getCrewsListJsonResult(sid);
		 Type list  = new TypeToken<ArrayList<Crew>>(){}.getType();
		 List<Crew> crews = new Gson().fromJson(json, list);
		return crews;
	}
